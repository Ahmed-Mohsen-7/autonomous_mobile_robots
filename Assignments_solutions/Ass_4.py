...
class PFLocalization():
    ...
    def run_localization(self, N, landmarks, iteration_num=18, sensor_std_err=.1, initial_x=None):
        ...
        if initial_x is not None:
            ...
            particles = self.create_uniform_particles((-20,20), (-20,20), (0, 6.28), N)
        weights = np.zeros(N)
...
pfl = PFLocalization(dt, std_vel=1, std_steer=np.radians(1))
# pfl.run_localization(100, landmarks, initial_x=(1,1, np.pi/4), iteration_num=500)
pfl.run_localization(500, landmarks, iteration_num=2000)
